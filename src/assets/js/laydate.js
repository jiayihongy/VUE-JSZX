	...
	C.prototype = [], C.prototype.constructor = C, w.extend = function() {
		...
		w(e.elem).on("click", function(e) {
			...
			w(n[0]).on("click", function(n) {
				e.change(t).prevYear();
				//				console.log(1111);
				for(var timein = 0; timein < timearry.length; timein++) {
					$("td[lay-ymd='" + timearry[timein] + "']").addClass("timeactive");
				}
				clearit();
			...
			var a = w(n).find("td");
			for(var jin = 0; jin < timearry.length; jin++) {
				$("td[lay-ymd='" + timearry[jin] + "']").addClass("timeactive");
			};
//			var aa = timearry.join(",");
//			$(".a").html(aa);
			clearit();
			$('td').unbind('click').click(function() {
				...
				if($(this).hasClass("timeactive")) {
					...
					var timedate = $(this).attr("lay-ymd");
					var n = timearry.indexOf(timedate);
					if(n != -1) {
						timearry.splice(n, 1);
                         
					}
					showtimeinput(timearry);
				} else {
					if($(this).attr("lay-ymd") != "a") {

						for(var i = 0; i < timearry.length; i++) {
							if(timearry[i] == $(this).attr("lay-ymd")) {
								return;
							}
						...
						timearry.push($(this).attr("lay-ymd"));
//						console.log(timearry);
//                       $(t.elem)
						showtimeinput(timearry);
						return;
			...
			$("body").keydown(function(event) {
				if(event.keyCode === 16) {
					$("td").mouseenter(function() {
						if($(this).hasClass("timeactive")) {} else {
							//							console.log(3434);
							if($(this).attr("lay-ymd") != "a") {
								for(var i = 0; i < timearry.length; i++) {
									if(timearry[i] == $(this).attr("lay-ymd")) {
...
function clearit() {
	$(".laydate-day-prev").attr("lay-ymd", "a");
	$(".laydate-day-prev").html("");
	$(".laydate-day-next").attr("lay-ymd", "a");
	$(".laydate-day-next").html("");
	$(".laydate-day-next").removeClass("timeactive");
	$(".laydate-day-prev").removeClass("timeactive");
}
function showtimeinput(arry){
//	util.qjarray = arry;
	var  allinit = '';
	allinit = arry.join(",");
...
//	}
//   console.log(domin);
     var  alltime = JSON.stringify(allinit);
     localStorage.setItem("allChiocetime", alltime);
     $(".datetest").html(allinit);
     $(".datetest").attr("title",allinit);
// $("#test2").val(allinit);
}
